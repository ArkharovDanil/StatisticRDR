...
namespace StatisticRDR
{
    public class StatForm12
    {
        ...
        {
            string path = "C:\\tempStatRDR\\StatForm12";
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            string fileName = "C:\\tempStatRDR\\StatForm12\\" + DateTime.Now.ToLongDateString() + "-" + ReturnAsNormalDate(date) + ".xls";
            ...
            try
            {
                ...
                workSheet.Cells[1, "A"] = "Распределение кол-ва перерегистрированных читателей по категориям читателей  и местам выдач за " + ReturnAsNormalDate(date);
                for (int i = 0; i < RowsCount; i++)
                {
